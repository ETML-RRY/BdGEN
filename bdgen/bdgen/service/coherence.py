...
def get_config_script_diff(name: str, output_root: Path | None = None) -> dict:
    ...
    empty: dict = {
        ...
        "removed": {"characters": [], "locations": [], "objects": []},
    }
    ...
    # Removed: present in the script but no longer in the config. These need a
    # cascading delete (drop + regenerate the pages that referenced them), so we
    # surface the impact (earliest affected page / pages dropped) per entity.
    config_char_ids = {c.id for c in config.characters}
    config_loc_ids = {l.id for l in config.locations}
    config_obj_ids = {o.id for o in config.objects}

    removed_chars = [sc for sc in bd_script.characters if sc.id not in config_char_ids]
    removed_locs = [sl for sl in bd_script.locations if sl.id not in config_loc_ids]
    removed_objs = [so for so in bd_script.objects if so.id not in config_obj_ids]
    ...
    def _impact(predicate) -> tuple[int | None, int]:
        """Return (earliest_affected_page, pages_dropped) for a removed entity."""
        earliest = None
        for p in sorted(bd_script.pages, key=lambda x: x.page_number):
            if any(predicate(panel) for panel in p.panels):
                earliest = p.page_number
                break
        total = len(bd_script.pages)
        dropped = 0 if earliest is None else max(0, total - (earliest - 1))
        return earliest, dropped

    def _removed_char_dict(c) -> dict:
        earliest, dropped = _impact(
            lambda panel: c.id in panel.characters or any(d.speaker == c.id for d in panel.dialogs)
        )
        return {"id": c.id, "name": c.name, "earliest_affected": earliest, "pages_dropped": dropped}

    def _removed_loc_dict(l) -> dict:
        earliest, dropped = _impact(lambda panel: panel.location == l.id)
        return {"id": l.id, "name": l.name, "earliest_affected": earliest, "pages_dropped": dropped}

    def _removed_obj_dict(o) -> dict:
        earliest, dropped = _impact(lambda panel: o.id in panel.objects)
        return {"id": o.id, "name": o.name, "earliest_affected": earliest, "pages_dropped": dropped}
    ...
    return {
        ...
        "removed": {
            "characters": [_removed_char_dict(c) for c in removed_chars],
            "locations": [_removed_loc_dict(l) for l in removed_locs],
            "objects": [_removed_obj_dict(o) for o in removed_objs],
        },
    }
...
def sync_script_with_config(
    ...
    script for these: only the affected entities and panels are touched.

    ``removals`` is an optional ``{"characters": [id, ...], "locations": [...],
    "objects": [...]}`` mapping of entities the user explicitly chose to drop
    from the script (they were removed from the config). Each confirmed removal
    is applied via the cascading delete (it truncates the pages that referenced
    the entity), and ``pages_dropped`` totals how many pages now need to be
    regenerated by the caller.

    Returns the same shape as ``apply_global_suggestion`` plus ``diff``,
    ``pages_dropped`` and per-kind removal counts.
    """
    ...
    # Keep only confirmed removals that the diff actually reports as removed —
    # guards against stale or spoofed ids coming from the client.
    removed_ids = {
        kind: {entry["id"] for entry in diff["removed"][kind]}
        for kind in ("characters", "locations", "objects")
    }
    confirmed = {kind: [] for kind in ("characters", "locations", "objects")}
    if removals:
        for kind in confirmed:
            for rid in removals.get(kind) or []:
                if rid in removed_ids[kind] and rid not in confirmed[kind]:
                    confirmed[kind].append(rid)
    removal_total = sum(len(v) for v in confirmed.values())

    if new_total + mod_total + removal_total == 0:
        return {"applied": False, "reason": "Aucun écart entre la configuration et le scénario.", "diff": diff}

    result: dict = {"applied": True, "changes": {}}

    # Integrate new / modified entities via the LLM (non-destructive).
    ...
    # Apply the confirmed removals via the cascading delete. Done after the LLM
    # integration so any surviving pages keep the freshly added entities; the
    # truncated tail is regenerated by the caller (which uses the current config).
    pages_dropped = 0
    changes = dict(result.get("changes") or {})
    if removal_total > 0:
        from .cascades import (
            delete_character_and_cascade,
            delete_location_and_cascade,
            delete_object_and_cascade,
        )

        for cid in confirmed["characters"]:
            info = delete_character_and_cascade(name, cid, output_root)
            pages_dropped += info.get("pages_dropped", 0)
        for lid in confirmed["locations"]:
            info = delete_location_and_cascade(name, lid, output_root)
            pages_dropped += info.get("pages_dropped", 0)
        for oid in confirmed["objects"]:
            info = delete_object_and_cascade(name, oid, output_root)
            pages_dropped += info.get("pages_dropped", 0)

        changes["character_removals"] = len(confirmed["characters"])
        changes["location_removals"] = len(confirmed["locations"])
        changes["object_removals"] = len(confirmed["objects"])

    return {**result, "applied": True, "changes": changes, "pages_dropped": pages_dropped, "diff": diff}
