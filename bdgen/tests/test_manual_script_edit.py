...
class ManualScriptEditTest(unittest.TestCase):
    def test_update_script_page_persists_and_marks_composed_page_stale(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            update_script_page_manual("demo", 1, new_page.model_dump(mode="json"), root)

            saved = BdGenScript.load(script_path)
            self.assertEqual(saved.pages[0].layout, "Une case modifiee.")
            self.assertEqual(saved.pages[0].panels[0].scene_description, "Nouveau texte.")
            self.assertEqual(saved.pages[0].panels[0].dialogs[0].text, "Nouveau dialogue.")
            self.assertEqual(read_stale_index(project_dir)["compose"], ["page_1"])
            self.assertTrue(read_coherence_index(project_dir)["dirty"])

    def test_update_script_page_rejects_page_number_change(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            script_path = root / "demo" / "bdgen-script.json"
            _save_script(_script(), script_path)
            payload = _script().pages[0].model_dump(mode="json")
            payload["page_number"] = 2

            with self.assertRaises(RuntimeError):
                update_script_page_manual("demo", 1, payload, root)

    def test_update_script_character_location_object_and_covers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            project_dir = root / "demo"
            script_path = project_dir / "bdgen-script.json"
            ...
            update_script_character_manual("demo", "perso_1", char_payload, root)

            loc_payload = _script().locations[0].model_dump(mode="json")
            loc_payload["description"] = "Nouveau decor."
            update_script_location_manual("demo", "decor_1", loc_payload, root)

            obj_payload = _script().objects[0].model_dump(mode="json")
            obj_payload["description"] = "Nouvel objet."
            update_script_object_manual("demo", "objet_1", obj_payload, root)

            cover_payload = _script().cover.model_dump(mode="json")
            cover_payload["scene_description"] = "Nouvelle couverture."
            update_script_cover_manual("demo", cover_payload, root)

            back_payload = _script().back_cover.model_dump(mode="json")
            back_payload["synopsis_blurb"] = "Nouveau synopsis."
            update_script_back_cover_manual("demo", back_payload, root)

            saved = BdGenScript.load(script_path)
            self.assertEqual(saved.characters[0].name, "Ada modifiee")
            self.assertEqual(saved.locations[0].description, "Nouveau decor.")
            self.assertEqual(saved.objects[0].description, "Nouvel objet.")
            self.assertEqual(saved.cover.scene_description, "Nouvelle couverture.")
            self.assertEqual(saved.back_cover.synopsis_blurb, "Nouveau synopsis.")
            self.assertCountEqual(read_stale_index(project_dir)["references"], ["decor_1", "objet_1", "perso_1"])
            self.assertCountEqual(read_stale_index(project_dir)["compose"], ["back", "cover"])
            # Dirty flag is set for any entity update
            self.assertTrue(read_coherence_index(project_dir)["dirty"])

    def test_add_script_character_location_object_rejects_duplicate_ids(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            project_dir = root / "demo"
            script_path = project_dir / "bdgen-script.json"
            ...
            saved = BdGenScript.load(script_path)
            self.assertEqual([character.id for character in saved.characters], ["perso_1", "perso_2"])
            self.assertEqual([location.id for location in saved.locations], ["decor_1", "decor_2"])
            self.assertEqual([obj.id for obj in saved.objects], ["objet_1", "objet_2"])
    ...
    def test_manual_edit_marks_coherence_dirty_and_check_flags_page_references(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            project_dir = root / "demo"
            script_path = project_dir / "bdgen-script.json"
            ...
            llm_result = {
                "issues": [
                    {"page_number": 1, "panel_number": 1, "kind": "character", "target": "perso_inconnu", "message": "Personnage inconnu."},
                    {"page_number": 1, "panel_number": 1, "kind": "object", "target": "objet_inconnu", "message": "Objet inconnu."},
                ],
                "suggestions": [],
            }
            with unittest.mock.patch("bdgen.service._llm_coherence_check", return_value=llm_result):
                coherence = check_script_coherence("demo", root)

            self.assertFalse(coherence["dirty"])
            self.assertEqual(coherence["flagged_pages"], [1])
            self.assertEqual({issue["target"] for issue in coherence["issues"]}, {"objet_inconnu", "perso_inconnu"})
            self.assertEqual(coherence["suggestions"], [])
